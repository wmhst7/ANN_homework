...
parser.add_argument('--cell', type=str, default="GRU",
    help='The kind of rnn cells')
args = parser.parse_args()

writer = SummaryWriter('./workspace/log')
...
if __name__ == '__main__':
    ...
    # model name
    mdname = args.cell + "layer_" + str(args.layers) + args.decode_strategy + "temp_" + str(args.temperature) + "prob_" \
             + str(args.max_probability) + "hidden_" + str(args.units)
    if not args.test:
        ...
        for epoch in range(1, args.num_epochs + 1):
            ...
            print("*******************************************************")
            # Plot
            writer.add_scalar(mdname+'/TrainLoss', train_loss, epoch)
            writer.add_scalar(mdname+'/ValidationLoss', val_loss, epoch)
            writer.add_scalar(mdname+'/ValidationPerplexity', val_ppl, epoch)
        ...
        with open('./workspace/'+mdname+'output.txt', 'w') as fout:
            for sent in result["gen"]:
                fout.write(" ".join(sent) + "\n")

        print(mdname + "        test_set, perplexity %.2f, forward BLEU %.3f, backward BLEU %.3f, harmonic BLEU %.3f" % (
        ppl, result["fw-bleu"], result["bw-bleu"], result["fw-bw-bleu"]))
        print("Test result: " + mdname + "    %.2f,  %.3f,  %.3f,  %.3f" % (
            ppl, result["fw-bleu"], result["bw-bleu"], result["fw-bw-bleu"]))
