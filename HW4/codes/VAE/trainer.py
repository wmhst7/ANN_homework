...
class Trainer(object):
    def __init__(self, device, model, optimizer, dataset, ckpt_dir, tb_writer, modelname):
        self._device = device
    ...
    def train(self, num_training_updates, logging_steps, saving_steps):
        ...
        fixed_noise = torch.randn(32, self._model.latent_dim)
        for i in tqdm(range(num_training_updates), desc='Training'):
            ...
            if (i + 1) % logging_steps == 0:
                self._tb_writer.add_scalar(self.mdname + "/reconstruction_loss", recon_loss, global_step=i)
                self._tb_writer.add_scalar(self.mdname + "/KL_divergence", kl_div, global_step=i)
                self._tb_writer.add_scalar(self.mdname + "/loss", loss, global_step=i)
            if (i + 1) % saving_steps == 0:
                ...
                self._tb_writer.add_scalar(self.mdname + '/dev/reconstruction_loss', eval_recon_loss, global_step=i)
                self._tb_writer.add_scalar(self.mdname + '/dev/KL_divergence', eval_kl_div, global_step=i)
                self._tb_writer.add_scalar(self.mdname + '/dev/loss', eval_recon_loss + eval_kl_div, global_step=i)
                for imgs, name in zip([dev_imgs, recons, samples], ['dev_imgs', 'reconstructions', 'samples']):
                    self._tb_writer.add_image(self.mdname + "/" + name, imgs, global_step=i)
                    save_image(imgs, os.path.join(dirname, "{}.png".format(name)))
        # Interpolation
        N = 10
        self._model.eval()
        noise1 = torch.randn(32, self._model.latent_dim)
        noise2 = torch.randn(32, self._model.latent_dim)
        for k in range(0, N):
            noise = noise1 + k * 1.0 / N * (noise2 - noise1)
            dev_imgs, recons, samples, eval_recon_loss, eval_kl_div = self.evaluate(noise)
            for imgs, name in zip([dev_imgs, recons, samples], ['dev_imgs', 'reconstructions', 'samples']):
                self._tb_writer.add_image(self.mdname + "/Interpolation" + name, imgs, global_step=k)
