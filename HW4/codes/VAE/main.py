...
if __name__ == "__main__":
    ...
    modelname = "VAE_latent_" + str(args.latent_dim) + "_batch_"+str(args.batch_size)+ "_numtrain_"+str(args.num_training_steps)
    ...
    if args.do_train:
        ...
        trainer = Trainer(device, model, optimizer, dataset, args.ckpt_dir, tb_writer, modelname)
    ...
    with torch.no_grad():
        ...
        while samples is None or samples.size(0) < num_samples:
            imgs = model.forward(z=torch.randn(args.batch_size, model.latent_dim))
            if samples is None:
                samples = imgs
            else:
    ...
    tb_writer.add_scalar(modelname+'/fid', fid)
